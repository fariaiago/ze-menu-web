defmodule ZeMenuSiteWeb.UsuarioRegistrationLiveTest do
  ...
  describe "Registration page" do
    test "renders registration page", %{conn: conn} do
      {:ok, _lv, html} = live(conn, ~p"/usuarios/register")
    ...
    test "renders errors for invalid data", %{conn: conn} do
      {:ok, lv, _html} = live(conn, ~p"/usuarios/register")

      result =
        lv
        |> element("#registration_form")
        |> render_change(usuario: %{"email" => "with spaces", "password" => "too short"})
    ...
    test "renders errors for duplicated email", %{conn: conn} do
      ...
      result =
        ...
        |> form("#registration_form",
          usuario: %{"email" => usuario.email, "password" => "valid_password"}
        )
        |> render_submit()
